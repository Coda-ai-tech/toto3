...
const inputDir = path.join(cwd, 'public', 'assets', 'img', 'sprites');
const inputDirRelative = path.relative(cwd, inputDir);
const typeDir = path.join(cwd, 'src', 'types');
const outputDir = path.join(cwd, 'public', 'assets', 'img');
const outputCssDir = path.join(cwd, 'src', 'styles');
...
async function generateIconFiles() {
  ...
  const spriteCssFilepath = path.join(outputCssDir, '_sprite.scss');
  ...
  const spriteCssChanged = await generateSvgSpriteCss({
    files,
    inputDir,
    outputPath: spriteCssFilepath,
  });
...
`
  ...
  if (spriteChanged || spriteCssChanged || typesChanged || readmeChanged) {
...
async function generateSvgSpriteCss({
  files,
  inputDir,
  outputPath,
}: {
  ...
  const symbols = await Promise.all(
    files.map(async (file) => {
      ...
      const template = `.svgIcon-${iconName(file)} {aspect-ratio:${svg.getAttribute('width')} / ${svg.getAttribute(
        'height'
      )};height: auto;}`;
      return template;
    })
  ...
  const output = [`@supports (aspect-ratio: 1 / 1) {`, ...symbols, `}`].join('\n');
