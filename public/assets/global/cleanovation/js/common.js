...
function start_global_slick() {
    $g_thumb_slick = $(".global__thumbs"), $g_thumb_slick.on("init", function (t, e, a) {
        ...
        $('.slick-current a').attr('tabindex','0');
        var curIndex = $('.slick-slide.slick-current a').parent().parent('.slick-slide').attr("data-slick-index");
        var curIndex_left01,curIndex_left02,curIndex_right01,curIndex_right02;
        curIndex_left01 = parseInt(curIndex) - 1;
        curIndex_left02 =  parseInt(curIndex) - 2;
        curIndex_right01 =  parseInt(curIndex) + 1;
        curIndex_right02 =  parseInt(curIndex) + 2;
        $('.slick-slide[data-slick-index='+ curIndex_left01 +'] a').attr('tabindex','0');
        $('.slick-slide[data-slick-index='+ curIndex_left02 +'] a').attr('tabindex','0');
        $('.slick-slide[data-slick-index='+ curIndex_right01 +'] a').attr('tabindex','0');
        $('.slick-slide[data-slick-index='+ curIndex_right02 +'] a').attr('tabindex','0');
        setTimeout(function () {
    ...
    }),  $(document).on('click','.global__sliderItem',function(){
        var wh = $(window).width();
        ...
        console.log(index);
    }),$(document).on('click','.thumbnail-list .thumbnail-item',function(){
        var wh = $(window).width();
        var ht = $(window).height();
        var sliderHeight = $("#g-slider").outerHeight();
        if(wh < 768){
    ...
    }),$g_slick.on('afterChange',function(event,slick, currentSlide){
